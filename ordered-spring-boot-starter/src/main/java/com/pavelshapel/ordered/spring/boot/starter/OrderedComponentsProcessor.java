...
public abstract class OrderedComponentsProcessor<P, R, C extends OrderedComponent<P, R>> implements Function<P, List<R>> {
    @Autowired
    ...
    @PostConstruct
    private void init() {
        orderedComponents = components.stream()
    ...
    private boolean isComponentPresentInProcessingOrder(C component) {
        ...
        return !isEmpty(classesInProcessingOrder) && classesInProcessingOrder.contains(component.getClass());
    }
